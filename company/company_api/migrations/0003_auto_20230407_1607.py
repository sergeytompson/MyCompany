...
def split_name(apps: Apps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    worker_model.objects.bulk_update(
        workers,
        (
            "last_name",
            "first_name",
...
def join_name(apps: Apps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    for worker in workers:
        patronymic_or_empty_line = f" {worker.patronymic}" if worker.patronymic else ""
        worker.name = (
            f"{worker.last_name} {worker.first_name}{patronymic_or_empty_line}"
        )
